...
func (b *Book) Trade() {
	...
	for order := range b.OrdersChan{ //para cada ordem que chegar no canal
		if order.OrderType == "BUY" { //se for uma ordem de compra
			buyOrders.Push(order) //adiciona na fila de ordens de compra
			if sellOrders.Len() > 0 && sellOrders.Orders[0].Price <= order.Price { //se tiver ordens de venda e o preço da ordem de venda for menor ou igual ao preço da ordem de compra
				sellOrder := sellOrders.Pop().(*Order) // remove a ordem de venda da fila e salva na variavel sellOrder
				if sellOrder.PendingShares > 0 { //se a ordem de venda tiver ações pendentes para vender
					...
					if sellOrder.PendingShares > 0 { // se a ordem de venda ainda tiver ações pendentes, adiciona ela na fila de ordens de venda
						sellOrders.Push(sellOrder) //adiciona a ordem de venda na fila de ordens de venda
			...
			sellOrders.Push(order) //adiciona na fila de ordens de venda
			if buyOrders.Len() > 0 && buyOrders.Orders[0].Price >= order.Price { //se tiver ordens de compra e o preço da ordem de compra for maior ou igual ao preço da ordem de venda
				buyOrder := buyOrders.Pop().(*Order) // remove a ordem de compra da fila e salva na variavel buyOrder
				if buyOrder.PendingShares > 0 { //se a ordem de compra tiver ações pendentes para comprar
					...
					if buyOrder.PendingShares > 0 { // se a ordem de compra ainda tiver ações pendentes, adiciona ela na fila de ordens de compra
						buyOrders.Push(buyOrder) //adiciona a ordem de compra na fila de ordens de compra
					}
				}
			}
		}
...
func (b *Book) AddTransaction(transaction *Transaction, wg *sync.WaitGroup) { //recebe uma transação e o waitgroup para gerenciar as threads quando terminar a operação.
	...
	b.Transaction = append(b.Transaction, transaction) //adiciona a transação no book
}